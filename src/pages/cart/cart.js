...
const Cart = () => {
    ...
    const handlerSubmit = async (value) => {
        ...
        try {
            ...
            await axios.post("http://localhost:5000/api/bills/addbills", newObject);
  ...
  return (
    <Layout>
      ...
      <div className="subTotal">
        <h2>Sub Total: <span> {(subTotal).toFixed(2)} DT</span></h2>
      ...
      <Modal title="Create Invoice" visible={billPopUp} onCancel={() => setBillPopUp(false)} footer={false}>
        <Form layout='vertical' onFinish={handlerSubmit}>
            ...
            <div className="total">
                <span>SubTotal: {(subTotal.toFixed(2))} DT</span><br />
                <span>TVA: {((subTotal / 100) * 3).toFixed(2) }DT</span>
                <h3>Total: {(Number(subTotal) + Number(((subTotal / 100) * 10).toFixed(2))).toFixed(2)} DT</h3>
            </div>
